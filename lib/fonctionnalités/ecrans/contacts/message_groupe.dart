...
class Discussion2Page extends StatefulWidget {
  final Discussion? discussion;
  final Contact? contact;
  final List<Contact>? contacts; // Ajout pour les messages de groupe

  const Discussion2Page({
    Key? key,
    this.discussion,
    this.contact,
    this.contacts,
  ...
  @override
  _Discussion2PageState createState() => _Discussion2PageState();
}

class _Discussion2PageState extends State<Discussion2Page> {
  ...
  void initState() {
    super.initState();
    // Vérifier le type de message/discussion
    isGroupMessage = widget.contacts != null && widget.contacts!.isNotEmpty;
    isNewMessage = (widget.contact != null || isGroupMessage) && widget.discussion == null;
  }
  ...
  // Obtenir le nom des destinataires pour l'affichage
  String _getRecipientNames() {
    if (isGroupMessage && widget.contacts != null) {
      return widget.contacts!.map((c) => c.name).join(", ");
    } else if (widget.contact != null) {
      return widget.contact!.name;
    } else if (widget.discussion != null) {
      return widget.discussion!.title;
    }
    return "";
  }
  ...
  // Construire l'interface de nouveau message (individuel ou groupe)
  ...
  Widget build(BuildContext context) {
    ...
    String appBarTitle;
    if (isNewMessage) {
      appBarTitle = isGroupMessage ? 'Nouveau Message de Groupe' : 'Nouveau Message';
    } else {
      appBarTitle = widget.discussion?.title ?? _getRecipientNames();
    }
